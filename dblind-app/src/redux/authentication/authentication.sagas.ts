...
type Params = { type: string; payload: { username: string; password: string } };

export function* signInStartAsync(action: Params) {
...
export function* signOutStartAsync() {
  try {
    ...
    yield console.log(`Start cleaning transactions`);
  } catch (error) {
